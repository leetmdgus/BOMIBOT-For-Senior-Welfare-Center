...
from app.common.domain.scoped_ids import scope_id, strip_scope
from app.modules.kanban.static_config import COLUMN_COLORS, COLUMN_TYPES
...
class SqlAlchemyKanbanBoardRepository(KanbanBoardRepository):
    ...
    def _get_task(self, region_id: str, task_id: str) -> KanbanTaskModel | None:
        ...
        scoped_id = scope_id(region_id, raw)
        ...
        if ":" in str(task_id):
            legacy = self._session.get(
                KanbanTaskModel,
                str(task_id),
                options=[joinedload(KanbanTaskModel.category)],
            )
            if legacy and str(legacy.id).startswith(f"{region_id}:"):
                return legacy

        return self._session.scalar(
            select(KanbanTaskModel)
            .where(KanbanTaskModel.id == scoped_id)
            .options(joinedload(KanbanTaskModel.category))
        )
