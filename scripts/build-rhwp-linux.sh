...
#
# 사용: ./scripts/build-rhwp-linux.sh   그 다음   docker compose up -d --build
set -euo pipefail
...
[ -f "$rhwp/Cargo.toml" ] || { echo "rhwp 소스를 찾을 수 없습니다: $rhwp (Cargo.toml 없음)" >&2; exit 1; }
mkdir -p "$out"
...
if command -v cargo >/dev/null 2>&1; then
  ...
  # CARGO_TARGET_DIR 를 컨테이너 임시 경로로 두어 호스트 rhwp/target 을 건드리지 않는다.
  docker run --rm \
